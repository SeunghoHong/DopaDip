...
- **포커스 세션** — 카운트다운 휠로 1분부터 60분까지 정하고, 한 번에 한 세션만 진행한다.
- **앱 차단** — `FamilyActivityPicker`로 고른 앱만 차단한다(차단 앱, blocklist). DopaDip은 막지 않으므로,
  집중하는 중에도 앱을 열어 세션을 끝낼 수 있다.
- **자동 해제** — 세션이 끝나면 앱이 Shield를 직접 풀고, `DeviceActivitySchedule`이 백그라운드에서도
  정시에 해제한다.
- **차단 화면** — 차단한 앱을 열면 "집중 중 · 남은 N분"을 보여 주는 다크 화면이 뜬다.
- **집중 링** — 진행 중에는 원형 진행 링과 큰 카운트다운으로 남은 시간을 보여 준다.

### 로드맵 (v2)
사용량 리포트, 여러 프로파일, 세션 히스토리와 스트릭을 준비하고 있다.
...
- 타깃 **iOS 17.0+**.
- **실기기에서 동작**: Screen Time(권한·Shield·스케줄)은 실기기에서만 돌아간다.
  시뮬레이터에서는 목 클라이언트로 UI를 확인한다.
- App Store에 올릴 때는 Apple **Family Controls (Distribution)** entitlement 승인이 필요하다
